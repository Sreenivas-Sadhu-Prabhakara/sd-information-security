...
| **Asset Type** | Security Policy |
| **Control Record** | Security Policy Administrative Plan |
...
| POST | `/v1/security-policy-administrative-plan/initiate` | Initiate |
| GET | `/v1/security-policy-administrative-plan` | Retrieve (list) |
| GET | `/v1/security-policy-administrative-plan/{crId}/retrieve` | Retrieve |
| PUT | `/v1/security-policy-administrative-plan/{crId}/update` | Update |
| PUT | `/v1/security-policy-administrative-plan/{crId}/control` | Control — body `{"action": "suspend"\|"resume"\|"terminate"}` |
...
curl -X POST localhost:8080/v1/security-policy-administrative-plan/initiate -H 'content-type: application/json' -d '{"note":"hello"}'
```
...
```bash
mvn -B verify
docker build -t bian/sd-information-security:0.1.0 .
```
...
Exposed through the platform Gateway at path prefix `/sd-information-security` (Cilium Gateway API). Mesh policy (`CiliumNetworkPolicy`) allows: gateway ingress, same-area peers, Prometheus — everything else denied.
